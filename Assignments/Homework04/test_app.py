...
def test_movie_range(client):
    response = client.get('/movies?year_start=2020&year_end=2025')
    assert response.status_code == 200
    data = response.get_json()
    assert isinstance(data, list)
    assert len(data) > 0  # Ensure dataset is not empty
    ...
    for movie in data:
        ...
        assert 2020 <= year <= 2025  # Check that year is within the specified range
...
def test_movie_mpa(client):
    response = client.get('/movies/mpa?rating=PG-13')
    assert response.status_code == 200
    data = response.get_json()
    assert isinstance(data, list)
    assert len(data) > 0  # Ensure dataset is not empty
